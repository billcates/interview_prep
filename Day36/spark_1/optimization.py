...
def check_delta_table_health(table_path: str, ideal_min_mb=64, ideal_max_mb=1024):
    ...
    if avg_file_size_mb < ideal_min_mb:
        print(f"⚠️ Too many small files detected — RECOMMEND: `OPTIMIZE ZORDER BY`")
    elif avg_file_size_mb > ideal_max_mb:
        print(f"⚠️ Files are too large — may impact scan performance.")
    else:
        print(f"✅ File size is optimal ✅")
    
    # Partition Strategy
    if not partition_cols:
        print("⚠️ Table is not partitioned — consider partitioning if table > 200GB.")
    else:
